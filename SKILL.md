...
- Prefer narrow block widths by default instead of filling cell width. Start narrow, wrap text, then widen only if wrapped text would clip: `150-180` for short one-line Chinese labels, `170-200` for typical two-line Chinese labels, and `210-240` for long two-line labels with separators such as `/`. Only use `250+` when a label would clearly clip after sensible line breaks.
- Use adaptive line breaks before widening. For labels over roughly 7 Chinese characters, split into two concise lines; for labels with `/`, split around `/` or into semantic chunks. Do not keep a long single line just to justify a wider rectangle.
...
- For short labels such as `提交装修结果`, `兼职确认人设`, `进入待首联池`, prefer widths around `160-190`; for two-line labels like `兼职注册\n提交基础信息`, prefer `180-210` unless the rendered text clips.
