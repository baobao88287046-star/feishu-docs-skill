...
When refining a reference swimlane, preserve the reference process logic first. Fix connector implementation and spacing without inventing new transitions. Prefer object-bound connectors using `attached_object.id` with explicit `snap_to` edges (`right -> left`, `bottom -> top`) and avoid `snap_to: auto` in table swimlanes. Use grid-aligned layouts: align same-row nodes by centerline, align vertically adjacent handoff nodes by `x` center when there is no crossing/overlap/boundary collision, and keep repeated styles consistent. Use narrow adaptive width tiers instead of one global oversized width: `150-180` for short labels, `170-200` for typical two-line labels, `210-240` for long two-line labels, and `250+` only when text would clip after sensible line breaks. Keep block centers away from swimlane dividers. Decision branches need inline `是/否` connector captions (`caption_position_type: 0`) when supported, not floating labels outside the line. Keep feedback/rework dashed lines local to the same cell or actor column; avoid cross-column red dashed lines and use local rework blocks instead. Semantic colors must be preserved: green success blocks, orange reuse notes, red rejection/rework, yellow decisions, blue normal steps.
